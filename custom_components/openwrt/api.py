...
class SSHAccessPointClient:
    ...
    def __init__(
        ...
        *,
        port: int = 22,
        command: str = DEFAULT_SSH_COMMAND,
        timeout: int = 10,
    ) -> None:
        self._host = host
        self._username = username
        self._password = password
        self._port = port
        self._command = command or DEFAULT_SSH_COMMAND
    ...
    async def async_get_clients(self) -> list[dict[str, Any]]:
        ...
        output = await self._async_run_command()
        return self._parse_assoclist_output(output)

    async def _async_run_command(self) -> str:
        ...
        try:
            async with asyncssh.connect(
                ...
                result = await asyncio.wait_for(
                    conn.run(self._command, check=True), timeout=self._timeout
                )
